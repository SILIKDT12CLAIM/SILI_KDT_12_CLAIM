...
from backend.core.defects import DEFECT_CODES, DEFECT_CODES_BY_PART, label_of, part_label_of

# 더미(모델 미로드) 모드도 프레임 4종에서만 랜덤 선택 — 커넥터 누수 방지
_DUMMY_CODES = DEFECT_CODES_BY_PART["FRAME"]
    ...
    try:
        ...
        if model:
            try:
                ...
                defect_type = random.choice(_DUMMY_CODES)
...
def _run_classify(model, image_data: bytes) -> tuple:
    ...
    if model:
        try:
            ...
            return random.choice(_DUMMY_CODES), round(random.uniform(0.55, 0.92), 3), True, True
...
# 보고서 본문에서 제거할 AI/시스템 자기언급 키워드
_AI_MENTION_TERMS = ('AI', '인공지능', '신뢰도', '머신러닝', '딥러닝', 'YOLO', '자동 분류', '자동분류')

def _scrub_ai_mentions(text: str) -> str:
    """공식 보고서 본문에서 AI/시스템 자기언급이 포함된 문장을 제거한다."""
    if not text:
        return text
    # 마침표/물음표/느낌표 또는 줄바꿈 기준으로 문장 분리
    parts = re.split(r'(?<=[.!?])\s+|\n+', text)
    kept = [p.strip() for p in parts
            if p.strip() and not any(term in p for term in _AI_MENTION_TERMS)]
...
- 각 항목은 2~4문장으로 작성할 것
- 한국어로 작성할 것
- 품질보증 담당자가 직접 작성한 공식 부적합 보고서다. 'AI', '인공지능', '신뢰도', '모델', '자동 분류', '머신러닝' 등 분석 도구나 시스템에 대한 언급을 절대 포함하지 말 것. 오직 불량 원인·조치·예방에 대한 실무 내용만 작성할 것
출력:"""
    ...
    async def event_generator():
        ...
        try:
            ...
            if result is None:
                ...
                return
            # 안전망: 보고서 본문에 AI/시스템 자기언급이 섞이면 해당 문장 제거
            for _k in ("root_cause_analysis", "corrective_action", "preventive_action"):
                if isinstance(result.get(_k), str):
                    result[_k] = _scrub_ai_mentions(result[_k])
